...
select customers.fullname_customer,
       acc.name_accompanied_service  as `name accompanied service`,
       acc.price_accompanied_service as price,
       acc.availability_status       as `status`
        ,
...
where customers.id_type_of_customer = 1
  and customers.address_customer in ('Quảng Nam ', 'Quảng Trị')