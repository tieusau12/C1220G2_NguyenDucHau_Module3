...
select contracts.id_contract , employees.fullname_employee , customers.fullname_customer, 
		customers.phone_number as phone_customer, services.name_service ,count(*) as 'amount_accompanied_services'
        ,contracts.date_started_contract , contracts.deposits
from employees
...
where contracts.id_contract in (
	select id_contract 
    from contracts 
    where ((month(date_started_contract) between 9 and 12) and year(date_started_contract)=2019) )
