...
values 
(1,'IP','Iphone',500,10,"Color Gold","available"),
(2,'Sam','Sam Sung',900,10,"Color Silver","unavailable");

alter table products add index idx_productCode(product_code);
	explain select * from products where product_code="Sam";
	
alter table products add index idx_product_name_price(product_name,product_price);
	explain select * from products where product_name='Iphone' or product_price ='900';
...
begin
	insert into products
    values 
		(3,'Mi','Xiaomi',400,0,"Color Red","unavailable");
end //
delimiter ;
